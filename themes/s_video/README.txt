...
S-Video boasts a clean HTML5 structure with extensible CSS classes and IDs for
...
S-Video's goal is to provide themers the building blocks needed to get their
designs up and running quickly and simply.

S-Video is perfect if you want a simple, smart, and flexible theme starter.
...
S-Video utilizes SASS for adaptive grids and layouts and general structure of the
...
S-Video is meant to be YOUR theme. Follow one of the two methods below and you can
rename 's_video' to another name like 'mytheme'. You're also welcome to keep
's_video'.

1. DRUSH: The provided drush install script will duplicate the s_video theme
   folder and rename all appropriate files and content to the new theme name you
   provide.

  - Download and enable the s_video theme: $ drush en s_video
  - Set the theme as default $ drush config-set system.theme default s_video
    or copy the includes/s_video.drush.inc into your .drush folder.
  - Run the provided drush install script: $ drush s_video-install
  ...
  - At this time, you can uninstall s_video: $drush pmu s_video
...
2. MANUAL: To manually change the name of the theme follow these steps BEFORE
  ...
  - Rename s_video.info.yml to mytheme.info.yml
  - edit s_video.info.yml and change the name, description, project (can be
    deleted), and change all other instances of 's_video' to 'mytheme'
  - Rename s_video.libraries.yml to mytheme.libraries.yml
  - Rename s_video.theme to mytheme.theme
  - in s_video.theme, change each instance of 's_video' to 'mytheme'
  - Rename config/schema/s_video.schema.yml to mytheme.schema.yml
  - Rename each file in config/install from block.block.s_video_tools.yml (for
    example) to block.block.mytheme_tools.yml
  - Every file in config/install, change each instance of 's_video' to
    'mytheme'
  - In js/source/scripts.js, change each instance of 's_video' to 'mytheme'
  - In theme-settings.php, change each instance of 's_video' to 'mytheme'
  - In templates/html.html.twig, change each instance of 's_video' to 'mytheme'
  - In templates/menu-local-tasks.html.twig, change each instance of 's_video' to
    'mytheme'
  - In templates/status-messages.html.twig, change each instance of 's_video' to
    'mytheme'

  When renaming, remember the following:
  - Do not simply replace every instance of 's_video' in every file in the theme.
    Most of S-Video's dependencies use the word 's_video' somewhere and renaming
    these instances will cause S-Video to break in unpredictable ways.
  ...
  - If you don't bother renaming S-Video in the above locations, be advised that
    you will run into conflicts with other versions of S-Video on your site. If
    your site uses more than one theme based on S-Video, make sure at least one of
    the themes has been renamed properly!


============================
How to compile SASS in S-Video
============================

To use SASS and automatically compile it within your theme, please refer to "How
to Use Grunt with S-Video" in the documentation below.
...
- build/scripts.js & source/scripts.js
  ...
  folder. See comments in s_video.libraries.yml file to enable the starter
  scripts.js file.
...
The layout used in S-Video is fairly similar to the Holy Grail method. It has been
...
This is how the page template is buit in S-Video, and it works in fluid and fixed
...
Once Node.JS is installed, go to the root folder of S-Video and install your Grunt
packages:
...
Once installed, cd to the root folder of S-Video and run Grunt via the command
line:
...
Once you start using S-Video, you will massively change it until you reach the
point where it has nothing to do with S-Video anymore. Unlike Zen, S-Video is not
...
version of S-Video comes out. Always see S-Video as a STARTER, and as soon as you
start using it, it is not S-Video anymore, but your own theme.

If you didn't rename your theme, but you don't want to be notified when S-Video
has a new version by the update module, simply delete "project = "s_video" in
s_video.info.yml.
...
Thanks for using S-Video, and remember to use the issue queue in drupal.org for
any questions or bug reports:

http://drupal.org/project/issues/s_video
